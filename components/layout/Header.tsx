...
export function Header() {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border transition-opacity duration-500 ${
          ...
          <div className="flex items-center justify-between h-auto py-4">
            ...
            >
              <div className="relative w-6 h-5">
                <span
                  className={`absolute left-0 w-full h-0.5 bg-brand-primary rounded-full transition-all duration-300 ${
                    isMenuOpen ? "top-1/2 -translate-y-1/2 rotate-45" : "top-0 rotate-0"
                  }`}
                />
                <span
                  className={`absolute left-0 top-1/2 -translate-y-1/2 w-full h-0.5 bg-brand-primary rounded-full transition-all duration-300 ${
      ...
      >
        <div
          className={`container mx-auto px-4 pt-24 pb-8 transition-transform duration-300 ${
        ...
        >
          <nav className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4 mt-8">
              ...
              <Button size="lg" className="w-full bg-brand-primary text-primary-foreground font-bold">
                <Link href="tel:+4194075670">Order Now</Link>
              </Button>
            </div>
          </nav>
